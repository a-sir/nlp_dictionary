 ...
 * Date: 10/23/13
 */
public class RuWiktionaryDumpHandler  extends DefaultHandler {
	...
	public RuWiktionaryDumpHandler(@NotNull Writer bw) {
		resetState();
		this.bw = bw;
	}
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		switch (qName) {
			...
			case "text":
				try {
					List<Cognem> cns = RuWiktionaryParseTool.parseDescription(title, sb.toString());
					if (cns.size() > 0) {
						if (firstLine) {
							firstLine = false;
						} else {
